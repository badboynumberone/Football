      ...
      success: function success(res) {
        ...
        if (IMG_RATIO >= 1) {
          _this.setData({
            ...
            cutL: Math.ceil((SCREEN_WIDTH - SCREEN_WIDTH*((res.height/res.width)-0.1)) / 2),
            cutT: 0,
            cutR: Math.ceil((SCREEN_WIDTH - SCREEN_WIDTH*((res.height/res.width)-0.1)) / 2),
            cutB: 0,
            // 图片缩放值
          ...
          _this.setData({
            ...
            cutL: 0,
            cutT: Math.ceil((_this.data.cropperH - _this.data.cropperH*((res.width/res.height)+0.1)) / 2),
            cutB: Math.ceil((_this.data.cropperH - _this.data.cropperH*((res.width/res.height)+0.1)) / 2),
            cutR: 0,
            // 图片缩放值
            scaleP: IMG_REAL_W / SCREEN_WIDTH,
            qualityWidth: DRAW_IMAGE_W,
    ...
    ctx.draw(true, () => {
      ...
      wx.canvasToTempFilePath({
        ...
        success: function (res) {
          console.log(res)
          if(res){
            app.globalData.itchImage = res.tempFilePath
            wx.hideLoading();
            wx.navigateBack({
              delta: 1
            }); 
